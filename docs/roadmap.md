...
分角色基线（v2，`make analyze-experiment` 的最后一张表）：

| 角色 | n | gold | 调检索率 |
| --- | --- | --- | --- |
| sunquan | 17 | 0.276 | 1.00 |
| simayi | 17 | 0.276 | 0.82 |
| liubei | 46 | 0.207 | 0.91 |
| zhouyu | 13 | 0.192 | 0.92 |
| caocao | 39 | 0.179 | 1.00 |
| zhugeliang | 53 | 0.104 | 0.94 |

几个角色 n 只有 13-17，分角色的 gold 噪声很大，别单独拿出来下结论。
v1 的同一张表已作废——换评测集之后旧的分层数字不可比（详见模块 6 第 1 条）。
...
决定（2026-09-21）：**美术和前端暂缓**，优先级不如下面那批从评估里长出来的活。
调研结论见本节末尾，要做的时候不用重新查。

待办，按"有依据 + 可测量"排序——这四条都是模块 4 的评估量出来的，
改完能用同一套评测集出前后对比：

1. **工具漏触发**。`tool_called` 0.941，整体还剩 6% 余量（196 题里约 12 题）。

   **这里有一条被推翻的归因，记下来免得重蹈。**最初在 v1 实验里看到
   司马懿调检索率只有 0.59，我判断是角色卡在压制工具调用——他被写成
   「回答之前先问对方为何而问……有时问完就不答了」。这个解释讲得通、
   听起来也漂亮，但**大部分不成立**：拉出 v2 的分角色数据一看，他在
   任何 agent 改动之前就已经爬到 0.82 了，靠的只是题目变具体。0.59 主要
   是 v1 评测集的产物（题目含糊，模型当闲聊），不是人设效应。
   而且 n=17、比例 0.82，标准误约 0.09，区间 0.64–1.00，**跟其他人那一簇
   0.91–1.00 已经重叠，统计上分不开了**。

   这个纠正是因为先跑了 `analyze-experiment` 拿 v2 基线才发现的。要是直接
   拿 v1 的 0.59 去比新实验，会得出"docstring 一改 0.59 → 0.9+，假说验证"
   这种完全错误但很好看的结论。**换了评测集之后，旧的分层数字一律作废。**

   真要改的时候，思路记在这里（试过一版又回退了，因为余量只剩 6%、
   测不出来，不是因为改得不对）：

   - 「**检索是你自己回想，不是对外说话**」——为司马懿写的。他可以先查
     清楚自己记得什么，然后照样反问、照样不答。查与不答本来不冲突，是
     模型把两者混为一谈了。这个改法**不用动任何角色卡**，人设完整保留。
   - 「对方问得随意、口语、像闲聊——只要话里有具体人事时地，照查」
   - 「问看法评价，先查事再评」——原来那条"问你的看法……不在史册"太宽，
     把"你觉得赤壁打得怎么样"也挡掉了
   - "不要用"那侧要加限定："纯寒暄，**话里没有任何具体史事**"，免得矫枉过正

   **注意取舍**：鼓励多查会让角色更像客服、更不像人，要盯
   `answer_relevance`（基线 0.908）有没有掉。这个风险目前比收益更值得测。
2. **检不到就脑补**。人工复核发现模型会先承认"检索里没有"，紧接着用先验
   知识往下编。角色卡缺一条"检索无果就明说记不清，不得以先验补"。
3. **双语料 + 出处标注**。`extract.py` 的三国志分支完整保留着，
   `settings.CORPUS` 一个开关就能开。目标是角色能区分「史载如此」和
   「世人传言」——玩家问空城计，诸葛亮答「此乃后人附会，亮未尝有此事」。
   工程上是多源 RAG + 出处冲突处理。
   **现有评测集测不出来**（题目全来自演义），要另建一个 30-40 条的
   「正史 vs 演义冲突」集。
4. **近形专有名词的精确匹配**。最差样本里 25% 栽在"夏侯霸 vs 夏侯和"这类
   字面极近、指代不同的人名上，稠密向量分不开、BM25 能分开。但受影响
   样本只有 3%，**做完测不出统计显著的提升**——按机制决定，不按数字决定。
5. **`configure_opik()`**。现在 Opik 的 API key 是 `config.py` 的导入副作用
   设的，写一个没导 config 的脚本就 401。换成显式调用。
6. **多玩家**。`thread_id` 现在就是 `character_id`，所有玩家共用一条对话。
   改成 `<player_id>-<character_id>`。

美术调研结论（2026-09-21，试过一轮之后记下来的）：

- **现有素材是 LPC（Liberated Pixel Cup）出的。**证据：atlas 里每个方向
  正好 9 帧走路（`walk-0000..0008`），这是 LPC 的标准规格；原始帧 64×64，
  philoagents 裁剪重打包成了约 30×46。
- **要换美术，用 LPC 的网页生成器**，不要自己画也不要找别的素材源：
  https://liberatedpixelcup.github.io/Universal-LPC-Spritesheet-Character-Generator/
  点选身体/发/须/冠帽/袍服，下载整张 spritesheet，**规格零转换**。
  仓库里是分层 PNG，理论上能程序化合成，但 GitHub API 和仓库 zip 都被
  出口代理挡了（403），单个 raw 文件能取却列不出目录，所以走网页版。
- **许可证**：LPC 大部分是 CC-BY-SA 3.0 / GPL 3.0，生成器会一起下载
  credits 文件，**必须随仓库提交**。传染性只作用于美术资源，不影响代码。
- **配色方案**（30 像素宽的画面里，颜色是唯一可靠的区分，不要六个人都穿甲）：
  诸葛亮白、刘备青、曹操黑红、孙权紫、周瑜碧、司马懿灰。
- 试过让 Claude 逐像素生成，三版之后放弃：放大能看的细节缩到实际尺寸
  全糊成一团，上限是"配色一致、剪影能区分"，达不到街机手绘的观感。
  **真正提升辨识度最划算的其实是头顶名牌**，一行代码，胜过二十版 sprite。
